...
const {addFS , updateFS, deleteFS,getDataFS}=useFirestore();
...
console.log(currentUser.uid);
  ...
  function removeChield(index) {
    console.log("you tryed to delete item num:",index);
    console.log(childsForm);
    const updatedForm = childsForm.filter((item,i)=> i !== index)
    setChildsForm(updatedForm);
    console.log(childsForm);
    console.log(childrenFormState);

    const updatedChildrenForm = [...childrenFormState];
    updatedChildrenForm.splice(index, 1);
    setChildrenFormState(updatedChildrenForm);

    console.log("second check: you tryed to delete item num:",index);
console.log(childrenFormState);
    if (NumOfChildren > 0) {
